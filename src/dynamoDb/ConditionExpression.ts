...
export default class ConditionExpression {
  ...
  isIn(list: AttributeValue[]): ConditionExpression {
    ...
    const attributes = this.attributes.copy();
    const left = attributes.addAttributeName(this.leftArgumentName);
    const listPlaceholders = list.map((value) => attributes.addAttributeValue(this.leftArgumentName, value));
  ...
  private copyAddingComparison(comparator: Comparator, right: AttributeValue): ConditionExpression {
    this.assertStringOperand(comparator);
    const attributes = this.attributes.copy();
    const leftPlaceholder = attributes.addAttributeName(this.leftArgumentName);
    const rightPlaceholder = attributes.addAttributeValue(this.leftArgumentName, right);
    const expression = `${leftPlaceholder} ${comparator} ${rightPlaceholder}`;
    return this.copyAddingExpression(expression, attributes);
  }

  private copyAddingFunction(functionName: string, arg?: any): ConditionExpression {
    ...
    const expression = `${functionName} (${name}${argPlaceholder ? `, ${argPlaceholder}` : ''})`;
    return this.copyAddingExpression(expression, attributes);
  }

  private copyAddingExpression(expression: string, attributes: ExpressionAttributeHelper): ConditionExpression {

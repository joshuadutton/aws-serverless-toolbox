...
import { DocumentClient, ScanInput, BatchGetRequestMap, ScalarAttributeType, QueryInput, ExpressionAttributeNameMap, ExpressionAttributeValueMap, BatchWriteItemRequestMap } from "aws-sdk/clients/dynamodb";
...
export default class DynamoDbWrapper {
  ...
  async query(table: string, keyExpression?: ConditionExpression, conditionExpression?: ConditionExpression, ExclusiveStartKey?: Key, items: any = []) {
  ...
  static cloudFormationForTable = function(name: string, primaryKey: { name: string, type: ScalarAttributeType }, secondaryKey?: { name: string, type: ScalarAttributeType }): CloudFormation {
  ...
  private async batchGetWithRequestMap(table: string, requestMap: BatchGetRequestMap, items: any[] = []) {
    return this.db.batchGet({ RequestItems: requestMap }).promise().then(data => {
      if (!data.Responses) {
        return items;
      }
  ...
  private async batchWriteWithRequestMap(table: string, requestMap: BatchWriteItemRequestMap) {
    return this.db.batchWrite({ RequestItems: requestMap }).promise().then(data => {
